...
--3.a.	%Resolução = soma do total de “Yes” dividido pela soma do total de “Yes” mais “No”
SELECT
	SUM(CAST(Resolução AS FLOAT)) AS QTD_YES,
	COUNT(Resolução) AS TOTAL,
	ROUND((SUM(CAST(Resolução AS FLOAT)) / NULLIF(COUNT(Resolução), 0)) * 100,2) AS [%Resolução]
FROM [BD].[dbo].[FatoCasosTrabalhados]

--3.b.	Total de casos fechados (com status “Done”)
SELECT
	COUNT(*) AS QTD
FROM [BD].[dbo].[FatoCasosTrabalhados]
WHERE ID_STATUS = 6 -- DONE
--3.c.	Total de Casos abertos
SELECT
	COUNT(*) AS QTD
FROM [BD].[dbo].[FatoCasosTrabalhados]
WHERE ID_STATUS <> 6 -- DONE
--3.d.	Tempo médio para atualização em Horas
SELECT
	AVG(DATEDIFF(HOUR,Data_Hora_Criação,Data_Hora_Atualização)) as TEMPO_MÉDIO_ATT
FROM [BD].[dbo].[FatoCasosTrabalhados]

--3.e.	Tempo médio para fechamento em Horas
SELECT
	AVG(DATEDIFF(HOUR,Data_Hora_Fechamento,Data_Hora_Criação)) as TEMPO_MÉDIO_FECHAMENTO
FROM [BD].[dbo].[FatoCasosTrabalhados]

--3.f.	Na tabela fato não é necessário trazer o ID do caso, logo será necessário sumarizar a tabela utilizando GROUP BY
